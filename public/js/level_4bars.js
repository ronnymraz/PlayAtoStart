var v = function(p){
	...
	var Square = function(num, offset){
		...
		this.dimension = p.createVector(100+Math.random()*20, 400+Math.random()*20);
		this.startColor = p.color(Math.random()*0, Math.random()*5, Math.random()*0, 100);
		this.currentColor = this.startColor;
		if(this.offset < 0)
			this.targetColor = p.color(Math.random()*55 + 190, Math.random()*55 + 160, Math.random()*55 + 60, 100);
		else
			this.targetColor = p.color(Math.random()*55 + 30, Math.random()*55 + 60, Math.random()*55 + 155, 100);
		this.alpha = 50;
		this.colorVal = 0;
		this.colorInc = 0.1;
		...
		this.react = function(){
			...
			p.scale(5);
			// p.triangle(0, (Math.cos(this.oscillation)+4)*20, -this.dimension.x*0.75, this.dimension.y, this.dimension.x*0.75, this.dimension.y);
			p.rect(this.num*20, height*6, 80, height*16);
...
function SelectRhythm(){
	var subdivisions = {};
	subdivisions.subdivisions0 = [2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 1, 1];
	subdivisions.subdivisions1 = [1, 1, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1];
	subdivisions.subdivisions2 = [1, 2, 1, 2, 1, 2, 2, 2, 1, 2, 1];
